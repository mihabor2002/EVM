...
void print_itog(Point Z, double y)
{
...
double Interpolation::funk(Field* TheField, Point Z)
{
    k_sredn alg;
    vector <Point> points, cluster, neighbouring_points, points_1;
    int i, j, k=0;
    double distant = 100, h = 0.1, sum_w = 0, sum_w_y = 0, r = 0, sum_y = 0, sum_eps = 0, mean_y = 0;
    ...
    else
    {
        ...
        for (i = 0; i < neighbouring_points.size(); i++) w.push_back(function(dist(Z,neighbouring_points[i]) / h));
        ...
        r = 1 - sum_eps / sum_y;
        cout << "Real: " << Z.get_function_value() << ". Forecast: " << sum_w_y / sum_w << ". r^2=" << r << "." << endl;
